...
Before confirming, parse `.specify/extensions/time-machine/features-queue.yml` as YAML and check:

- `total_features` equals the number of entries in `features`.
- Every feature has the required keys from the template.
- Every path listed under `files` exists relative to `project_root`.
- Feature ids are lowercase URL-safe ids using only letters and hyphens.

If `.specify/` is untracked in git, `git diff` may not show the new queue. Do not treat that as a failure; rely on direct file existence and YAML validation, then mention the untracked tree in the summary if relevant.
