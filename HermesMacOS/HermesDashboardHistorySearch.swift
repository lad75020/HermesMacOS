...
struct HermesDashboardConversationResult: Identifiable, Decodable {
    ...
    var sessionFriendlyName: String {
        ...
        if !normalizedPrompt.isEmpty { return normalizedPrompt }
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case session, matches, messages, metadata, title
    ...
    private static func metadataTitle(_ metadata: [String: HermesFlexibleJSONValue]?) -> String? {
        ...
        if let title = titleKeys.compactMap({ metadata[$0]?.readableText.trimmingCharacters(in: .whitespacesAndNewlines) }).first(where: { !$0.isEmpty }) { return title }
        for nestedKey in ["session", "conversation"] {
            if let nested = metadata[nestedKey]?.objectValue,
               let title = titleKeys.compactMap({ nested[$0]?.readableText.trimmingCharacters(in: .whitespacesAndNewlines) }).first(where: { !$0.isEmpty }) { return title }
        }
        return nil
    }
}
...
struct HermesDashboardSessionInfo: Decodable {
    ...
    init(from decoder: Decoder) throws {
        ...
        title = directTitle ?? metadata.flatMap { metadata in
            ["title", "display_title", "friendly_name", "name", "summary"].compactMap { metadata[$0]?.readableText.trimmingCharacters(in: .whitespacesAndNewlines) }.first { !$0.isEmpty }
        }
        ...
        profile = directProfile ?? metadata.flatMap { metadata in
            ["profile", "profile_name", "profile_id", "hermes_profile"].compactMap { metadata[$0]?.readableText.trimmingCharacters(in: .whitespacesAndNewlines) }.first { !$0.isEmpty }
        }
    }
}
...
struct HermesDashboardMessageMatch: Identifiable, Decodable {
    ...
    enum CodingKeys: String, CodingKey {
        case id, role, snippet, timestamp, source, model
        case sessionID = "session_id"
...
struct HermesDashboardConversationMessage: Identifiable, Decodable {
    ...
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        if let intID = try? container.decode(Int.self, forKey: .id) { id = String(intID) }
        else if let stringID = try? container.decode(String.self, forKey: .id) { id = stringID }
        else { id = UUID().uuidString }
        role = (try? container.decode(String.self, forKey: .role)) ?? "message"
        content = Self.decodeFlexibleContent(from: container) ?? ""
        timestamp = try? container.decode(Double.self, forKey: .timestamp)
        toolName = try? container.decode(String.self, forKey: .toolName)
    }

    private static func decodeFlexibleContent(from container: KeyedDecodingContainer<CodingKeys>) -> String? {
        if let string = try? container.decode(String.self, forKey: .content) { return string }
        if let array = try? container.decode([HermesFlexibleJSONValue].self, forKey: .content) { return array.map(\.readableText).filter { !$0.isEmpty }.joined(separator: "\n") }
        if let object = try? container.decode(HermesFlexibleJSONValue.self, forKey: .content) { return object.readableText }
        return nil
    }
}

private enum HermesFlexibleJSONValue: Decodable {
    case string(String), number(Double), bool(Bool), array([HermesFlexibleJSONValue]), object([String: HermesFlexibleJSONValue]), null
    ...
    var readableText: String {
        ...
        case .array(let values): return values.map(\.readableText).filter { !$0.isEmpty }.joined(separator: "\n")
        case .object(let object):
            if let text = object["text"]?.readableText, !text.isEmpty { return text }
            if let content = object["content"]?.readableText, !content.isEmpty { return content }
            return object.values.map(\.readableText).filter { !$0.isEmpty }.joined(separator: "\n")
        case .null: return ""
        }
    }

    var objectValue: [String: HermesFlexibleJSONValue]? { if case .object(let object) = self { return object }; return nil }
    ...
    init(from decoder: Decoder) throws {
        ...
        else if let value = try? container.decode([HermesFlexibleJSONValue].self) { self = .array(value) }
        else { self = .object((try? container.decode([String: HermesFlexibleJSONValue].self)) ?? [:]) }
    }
}
...
final class HermesDashboardHistorySearchSession {
    ...
    func search(dashboardBaseURL: String, apiSettings: HermesAPISettings, profileFilter: String = "all", limit: Int = 25) {
        ...
        guard !trimmedQuery.isEmpty else {
            requestTask?.cancel(); activeSearchID = nil; results = []; matchedMessages = 0; matchedSessions = 0; status = "Enter a search query"; lastErrorMessage = ""
            return
        }
        requestTask?.cancel()
        let searchID = UUID()
        ...
        requestTask = Task { await runSearch(searchID: searchID, dashboardBaseURL: dashboardBaseURL, apiSettings: apiSettings, query: trimmedQuery, profileFilter: profileFilter, limit: limit) }
    }
    ...
    private func runSearch(searchID: UUID, dashboardBaseURL: String, apiSettings: HermesAPISettings, query: String, profileFilter: String, limit: Int) async {
        ...
        do {
            ...
            if activeSearchID == searchID { status = "Cancelled" }
        } catch {
    ...
    private func filter(_ results: [HermesDashboardConversationResult], profileFilter: String) -> [HermesDashboardConversationResult] {
        ...
        return results.filter { normalizedProfileName($0.session.profile ?? "default") == selected }
    }
    ...
    private func fetchConversations(baseURL: URL, token: String, apiSettings: HermesAPISettings, query: String, profileFilter: String, limit: Int) async throws -> HermesDashboardConversationSearchResponse {
        ...
        var queryItems = [URLQueryItem(name: "q", value: query), URLQueryItem(name: "limit", value: String(limit)), URLQueryItem(name: "role", value: "user,assistant,tool")]
        if normalizedProfileName(profileFilter) != "all" { queryItems.append(URLQueryItem(name: "profile", value: profileFilter)) }
